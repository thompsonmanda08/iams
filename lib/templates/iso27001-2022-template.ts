 ...
 *
 * Note: This file now primarily serves as a fallback. Templates and categories
 * should be fetched from the database using the provided server actions and hooks.
 *
...
/**
 * ISO 27001:2022 complete workpaper template with all 11 categories (FALLBACK)
 * This is used as a fallback when DB data is not available
 */
export const ISO27001_2022_TEMPLATE: WorkpaperTemplateDefinition = {
  ...
  categories: [
...

    {
      ...
      group: "main-clauses",
      objectives:
        "Review organisation's determination of external and internal issues relevant to ISMS",
      scope:
        "ISO 27001:2022 clauses 4.1 (Understanding the organization and its context), 4.2 (Understanding the needs and expectations of interested parties), 4.3 (Determining the scope of the ISMS), and 4.4 (Information security management system)",
...
6. Verify alignment between business context and ISMS scope`,
      description:
        "Understanding the organization and its context, interested parties, and ISMS scope",
      isRequired: false
    },

    {
      ...
      group: "main-clauses",
      objectives:
        "Assess leadership commitment, information security policy, and organizational roles",
      scope:
        "ISO 27001:2022 clauses 5.1 (Leadership and commitment), 5.2 (Policy), and 5.3 (Organizational roles, responsibilities and authorities)",
...
7. Verify resource allocation for ISMS`,
...

    {
      ...
      scope:
        "ISO 27001:2022 clauses 6.1 (Actions to address risks and opportunities), 6.2 (Information security objectives and planning to achieve them), and 6.3 (Planning of changes)",
...
7. Review change management processes for ISMS`,
...

    {
      ...
      group: "main-clauses",
      objectives:
        "Assess resources, competence, awareness, communication, and documented information",
      scope:
        "ISO 27001:2022 clauses 7.1 (Resources), 7.2 (Competence), 7.3 (Awareness), 7.4 (Communication), and 7.5 (Documented information)",
...
7. Review documented information availability and protection`,
...

    {
      ...
      group: "main-clauses",
      objectives:
        "Review operational planning, information security risk assessment, and risk treatment implementation",
      scope:
        "ISO 27001:2022 clauses 8.1 (Operational planning and control), 8.2 (Information security risk assessment), and 8.3 (Information security risk treatment)",
...
7. Review outsourced processes and their control`,
...

    {
      ...
      scope:
        "ISO 27001:2022 clauses 9.1 (Monitoring, measurement, analysis and evaluation), 9.2 (Internal audit), and 9.3 (Management review)",
...
7. Review action items from management reviews`,
...

    {
      ...
      scope:
        "ISO 27001:2022 clauses 10.1 (Continual improvement) and 10.2 (Nonconformity and corrective action)",
...
7. Assess improvement opportunities identification`,
...

    {
      ...
      objectives:
        "Review implementation and effectiveness of organizational information security controls",
      scope:
        "ISO 27001:2022 Annex A, Section 5 - Organizational Controls (37 controls covering policies, organization, human resources, asset management, access control, supplier relationships, incident management, business continuity, compliance, and documentation)",
...
10. Assess business continuity and compliance (A.5.29-A.5.37)`,
      description:
        "Comprehensive organizational security controls including policies, HR security, asset management, and compliance",
      isRequired: false
    },

    {
      ...
      scope:
        "ISO 27001:2022 Annex A, Section 6 - People Controls (8 controls covering screening, terms and conditions, information security awareness, disciplinary process, and employment termination)",
...
8. Assess information security event reporting (A.6.8)`,
...

    {
      ...
      scope:
        "ISO 27001:2022 Annex A, Section 7 - Physical Controls (14 controls covering physical security perimeters, entry controls, securing offices, equipment security, and environmental protection)",
...
10. Assess storage media and equipment disposal (A.7.10-A.7.14)`,
...

    {
      ...
      scope:
        "ISO 27001:2022 Annex A, Section 8 - Technological Controls (34 controls covering user endpoints, access rights, authentication, cryptography, network security, system security, secure development, and testing)",
...
/**
 * Fetch all available workpaper templates from the database
 * Use this in server components or server actions
 */
export async function fetchAvailableTemplates(): Promise<WorkpaperTemplateDefinition[]> {
  try {
...
/**
 * Fetch template by ID from the database with categories
 * Use this in server components or server actions
 */
export async function fetchTemplateById(
  ...
  try {
    ...
    console.error(`Failed to fetch template ${templateId} from DB:`, error);
  }
...
/**
 * Fetch all categories for a specific template from the database
 * Use this in server components or server actions
 */
export async function fetchTemplateCategoriesById(templateId: string): Promise<TemplateCategory[]> {
  try {
    ...
    console.error(`Failed to fetch categories for template ${templateId} from DB:`, error);
  }

  // Fallback to static template categories
  return getTemplateCategoriesById(templateId);
}
 ...
 * Use this in server components or server actions
 */
export async function fetchCategoryById(categoryId: string): Promise<TemplateCategory | null> {
  try {
    ...
    console.error(`Failed to fetch category ${categoryId} from DB:`, error);
  }
...
// ============================================================================
// STATIC FALLBACK FUNCTIONS (Client-side compatible)
// ============================================================================

/**
 * Get all available workpaper templates (static fallback)
 * @deprecated Use fetchAvailableTemplates() for server-side or useWorkpaperTemplates() hook for client-side
 */
...
/**
 * Get template by ID (static fallback)
 * @deprecated Use fetchTemplateById() for server-side or useWorkpaperTemplatesWithCategories() hook for client-side
 */
export function getTemplateById(templateId: string): WorkpaperTemplateDefinition | null {
  const templates = getAvailableTemplates();
  return templates.find((t) => t.id === templateId) || null;
}

/**
 * Get all categories for a specific template (static fallback)
 * @deprecated Use fetchTemplateCategoriesById() for server-side
 */
export function getTemplateCategoriesById(templateId: string): TemplateCategory[] {
  const template = getTemplateById(templateId);
  return template?.categories || [];
}

/**
 * Get a specific category by template ID and category ID (static fallback)
 * @deprecated Use fetchCategoryById() for server-side
 */
export function getCategoryById(templateId: string, categoryId: string): TemplateCategory | null {
