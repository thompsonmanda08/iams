...
export function DataSourceSelector({
  ...
  const filteredByAllowedCategories = useMemo(() => {
    ...
    return dataSources.filter((ds) => filterByCategory.includes(ds.category));
  }, [dataSources, filterByCategory]);

  // Get available categories from filtered sources
  const availableCategories = useMemo(() => {
    return Array.from(new Set(filteredByAllowedCategories.map((ds) => ds.category)));
  ...
  return (
    <div className={cn("relative", className)}>
      ...
      <button
        ...
        className="flex w-full items-center justify-between gap-2 rounded-lg border border-gray-300 bg-white px-3 py-2.5 text-sm transition-colors hover:bg-gray-50 focus:border-blue-500 focus:outline-none focus:ring-1 focus:ring-blue-500"
      >
      ...
      {/* Dropdown */}
      {isOpen && (
        <div className="absolute top-full left-0 z-50 mt-1 w-full min-w-[320px] rounded-lg border border-gray-200 bg-white shadow-lg">
          {/* Search */}
                ...
                return (
                  ...
                  >
                    ...
                    <div className="mt-2 flex flex-wrap gap-1">
                      {source.compatible_widgets.map((widget) => (
                        <span
                          key={widget}
