...
export const PieChartWidget = ({
  ...
  return (
    <div className="border-border bg-card rounded-lg border p-4">
            ...
            {slices.map((slice, i) => (
              <div key={i} className="flex items-center gap-2">
                <input
                  ...
                  className="border-input bg-background h-8 w-8 cursor-pointer rounded border"
                />
                <Input
                  type="text"
                  value={slice.label}
      ...
      {slices.length === 0 ? (
        <WidgetEmptyState
          icon={<PieChartIcon className="text-muted-foreground mx-auto h-10 w-10" />}
          ...
          <ChartContainer config={chartConfig} className="h-56 w-56">
            <PieChart>
              ...
              <Pie
                ...
                activeShape={({ outerRadius = 0, fill, ...props }: PieSectorDataItem) => (
                  <g>
              ...
              return (
                <button
                  ...
                  className={`flex w-full items-center gap-2 rounded px-2 py-1 text-left text-sm transition-colors ${
                    isActive ? "bg-blue-50 font-medium dark:bg-blue-950" : "hover:bg-muted/50"
                  }`}>
                  ...
                  <span
                    className={`ml-auto ${isActive ? "text-foreground font-semibold" : "text-foreground font-medium"}`}>
                    ({slice.value})
                  </span>
                </button>
              );
