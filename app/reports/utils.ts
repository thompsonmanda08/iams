...
export const populateReportLogo = (sections: ReportSection[], logoUrl: string): ReportSection[] => {
  ...
  return sections.map((section) => {
    ...
    try {
      ...
      // Only update if it's currently a placeholder or empty, or different
      const currentLogo = content.organization?.logo_url;

      // If valid logo already exists and is not default/placeholder, skip?
      // User requirement was to auto-populate from session.
      // Let's assume we overwrite if it's the default or placeholder.
      if (
        currentLogo &&
