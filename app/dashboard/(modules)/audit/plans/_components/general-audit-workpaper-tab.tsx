...
// Tooltip-badge legend showing the evidence grid column structure.
// Rendered in both the locked state and the full render — extracted to avoid duplication.
function ConfigLegend({ config }: { config: any }) {
  return (
    <Card>
  ...
  return (
    <>
      <div className="space-y-4">
        {/* Column + key legend */}
        <ConfigLegend config={config} />

        {/* Evidence grid — locked when plan not yet approved */}
        {isDisabled ? (
        ...
        {/* Work Done & Conclusion — only shown when workpaper is unlocked */}
        {!isDisabled && (
          <Card>
            <CardHeader>
