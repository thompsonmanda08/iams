...
//     const pemKey = permission as keyof typeof permissionsMatrix;
//     setPermissionsMatrix((prev) => ({
//       ...prev,
//       [permission]: {
//         ...prev[pemKey],
//         [role]: !prev[permission as keyof typeof permissionsMatrix][role]
//       }
//     }));
//   };
...
type RoleId = "admin" | "organizer" | "moderator" | "editor" | "admitter" | "staff";
...
type RolePermissions = Record<RoleId, boolean>;
type PermissionsMatrix = Record<PermissionKey, RolePermissions>;
...
export default function RolesPermissions() {
  ...
  // Define permissions matrix with proper typing
  ...
  // Function to toggle permission with proper typing
  const togglePermission = (permission: PermissionKey, role: RoleId) => {
    setPermissionsMatrix((prev) => ({
      ...prev,
      [permission]: {
        ...prev[permission],
  ...
  // Group permissions by name with proper typing
  const getPermissionsByCategory = (name: string): PermissionKey[] => {
    return Object.keys(permissionsMatrix).filter((permission): permission is PermissionKey =>
  ...
  const hasPermission = (permission: PermissionKey, roleId: RoleId): boolean => {
    return permissionsMatrix[permission][roleId];
  };
                      ...
                      {getPermissionsByCategory(name).map((permission) => (
                        <TableRow key={permission}>
                          ...
                          {roles.map((role) => (
                            <TableCell key={role.id}>
                              <Switch
                                checked={hasPermission(permission, role.id)}
                                onCheckedChange={() => togglePermission(permission, role.id)}
                                disabled={role.id === "admin"} // Admin always has all permissions
                              />
                            </TableCell>
