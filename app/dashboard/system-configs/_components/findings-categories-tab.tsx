...
import { AuditConfigurableItem, Pagination } from "@/lib/types";
import {
  Dialog,
  DialogClose,
  DialogContent,
...
export default function FindingsCategoryTab({
  ...
  const [editingDepartment, setEditingDepartment] = useState<AuditConfigurableItem | null>(null);
  ...
  const deleteMutation = useMutation({
    ...
    onSuccess: (response) => {
      if (response.success) {
        notify({ description: "AuditConfigurableItem deleted successfully", type: "success" });
    ...
    <>
      <Card className="p-4">
        <div className="mb-4 flex items-center justify-between">
          <h3 className="text-lg font-semibold">Findings Category</h3>
          <Button
            size="sm"
            onClick={() => {
...
const INIT_AREA: AuditConfigurableItem = {
  id: "",
  name: "",
  description: "",
  department_id: null
...
export function CreateOrUpdateArea({
  ...
  openModal?: boolean;
  departmentId?: string;
  initialData?: AuditConfigurableItem | null;
  setInitialData?: React.Dispatch<React.SetStateAction<AuditConfigurableItem | null>>;
  ...
  const [formData, setFormData] = useState<AuditConfigurableItem>(() => {
  ...
  const saveMutation = useMutation({
    ...
    onSuccess: (response) => {
      if (response.success) {
        notify({ description: `AuditConfigurableItem ${initialData ? "updated" : "created"} successfully`, type: "success" });
        queryClient.invalidateQueries({ queryKey: [QUERY_KEYS.DEPARTMENTS] });
        setOpenModal?.(false);
        setInitialData?.(null);
        setFormData(INIT_AREA);
